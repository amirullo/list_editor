...
class ProjectRepository(BaseRepository[Project]):
    ...
    def get_all_for_user(self, user_internal_id: int) -> TypeList[Project]:
    ...
    def get_by_id_for_user(self, project_id: int, user_internal_id: int) -> Optional[Project]:
    ...
    def add_user_to_project(self, project: Project, user: User, role: ProjectRoleType) -> ProjectUser:
        project_user = ProjectUser(user_id=user.id, project_id=project.id, role_type=role)
        self.db.add(project_user)
        self.db.commit()
        self.db.refresh(project_user)
        return project_user

    def remove_user_from_project(self, project: Project, user: User) -> None:
