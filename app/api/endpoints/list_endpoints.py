...
async def get_list(
    ...
    list_service: ListService = Depends(get_list_service),
    user_id: str = Depends(get_user_id),
    _: None = Depends(require_list_access(0))  # 0 will be replaced by list_id in the dependency
):
    try:
        list_data = await list_service.get_list(list_id, user_id)
        return ResponseModel(data=list_data, message="List retrieved successfully")
    except BaseAPIException as e:
        raise HTTPException(status_code=e.status_code, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
...
async def update_list(
    ...
    list_service: ListService = Depends(get_list_service),
    user_id: str = Depends(get_user_id),
    _: None = Depends(require_list_creator(0))  # 0 will be replaced by list_id in the dependency
):
    try:
        updated_list = await list_service.update_list(list_id, list_update, user_id)
...
async def add_user_to_list(
    ...
    try:
        updated_list = await list_service.add_user_to_list(list_id, user_data, user_id)
...
async def remove_user_from_list(
    ...
    try:
        updated_list = await list_service.remove_user_from_list(list_id, user_id_to_remove, user_id)
...
async def create_item(
    ...
    try:
        created_item = await item_service.create_item(list_id, item_create, user_id)
...
async def get_items(
    ...
    try:
        items = await item_service.get_items_by_list(list_id, user_id)
...
async def update_item(
    ...
    try:
        updated_item = await item_service.update_item(list_id, item_id, item_update, user_id)
...
async def delete_item(
    ...
    try:
        result = await item_service.delete_item(list_id, item_id, user_id)
        return ResponseModel(data=result, message="Item deleted successfully")
    except BaseAPIException as e:
        raise HTTPException(status_code=e.status_code, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
...
async def acquire_lock(
    ...
    try:
        lock = await lock_service.acquire_lock(list_id, user_id)
...
async def release_lock(
    ...
    try:
        result = await lock_service.release_lock(list_id, user_id)
